...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.encoder = nn.LSTM(input_size=embed_size,
                               ...
                               bias=True, 
                               bidirectional=True)
        self.decoder = nn.LSTMCell(input_size=2*embed_size,
                                   hidden_size=hidden_size,
                                   bias=True)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        enc_hiddens, (last_hidden, last_cell) = self.encoder(packX)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        for batch in torch.split(Y, 1):
            Y_t = torch.squeeze(batch, dim=0)
            Ybar_t = torch.cat((Y_t, o_prev))
            (next_h, next_c), o_t, e_t = self.step(Ybar_t, dec_state, enc_hiddens, enc_hiddens_proj, enc_masks)
